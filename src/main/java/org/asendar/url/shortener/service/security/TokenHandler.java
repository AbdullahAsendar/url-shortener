...
public class TokenHandler {
	...
	public UserEntity parseUserFromToken(String token) {
		...
		if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
			try {
				...
				if (validHash) {
					final UserEntity user = fromJSON(userBytes);
					return user;
	 ...
	 * secret = new byte[70]; new java.security.SecureRandom().nextBytes(secret);
	 * TokenHandler tokenHandler = new TokenHandler(secret); for (int i = 0; i <
	 * 1000; i++) { final User user = new
	 * User(java.util.UUID.randomUUID().toString().substring(0, 8), new Date( new
	 * Date().getTime() + 10000)); user.grantRole(UserRole.ADMIN); final String
	 * token = tokenHandler.createTokenForUser(user); final User parsedUser =
